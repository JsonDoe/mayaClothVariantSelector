...
def get_enum_keys_dict(node_name):
    ...
    for attr in custom_attrs:
        if cmds.attributeQuery(
            attr, node=node_name, attributeType=True) == 'enum':
...
def set_enum_key_by_name(node_name, attr_name, key_name):
    ...
    if not cmds.objExists(full_attr):
        print(f"Attribute '{full_attr}' does not exist.")
        return

    enum_keys = cmds.attributeQuery(attr_name, node=node_name, listEnum=True)
    if not enum_keys:
        print(f"Attribute '{full_attr}' is not an enum.")
        return

    keys = enum_keys[0].split(':')
    if key_name not in keys:
        print(
            f"'{key_name}' not in enum values of '{full_attr}'. Valid: {keys}")
        return
...
def delete_all_keys_on_controller(controller_node):
    ...
    for attr in enum_attrs.keys():
        try:
            ...
            print(f"Failed to clear keys on {controller_node}.{attr}: {e}")
...
def update_controller_variants(controller_node, asset_list, logic_dict):
    ...
    for asset in asset_list:
        ...
        if cmds.attributeQuery(
                matched_attr, node=controller_node, exists=True):
            set_enum_key_by_name(controller_node, matched_attr, asset)
        else:
            print(
                f"Attribute '{matched_attr}' doesn't exist on '{controller_node}'.")
...
charaGloLo = [
    ...
    if ("M_globalLocal_setup_CON" in x or "M_golbalLocal_setup_CON" in x) and
       ("constance" in x or "romane" in x) and not x.endswith("Shape")
]
